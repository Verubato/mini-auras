-- The unit frame hooks are installed once and outlive the module going dormant, so the CC icons
-- come back on the raid frames in a battleground it was switched off for.
...
local held = env.addUnitFrame("party1", "CUF_CcZoneGate")
...
local function LeaveToWorld()
	db.Modules.CrowdControl.Enabled.BattleGrounds = true
...
fw.describe("CrowdControl - the zone gate", function()
	...
	fw.it("builds nothing for a frame that first gets its unit mid match", function()
		...
		local joined = env.addUnitFrame("party2", "CUF_CcZoneGateJoined")
	...
	fw.it("still takes a frame that gets its unit while the zone allows it", function()
		local joined = env.addUnitFrame("party4", "CUF_CcZoneGateAllowed")
		...
		env.addon.Modules.CrowdControl.Display:OnCufSetUnit(joined, "party4")
